...
import { ArrowLeft, ArrowRight, Upload, FolderOpen, BookOpen, Library, FlaskConical } from 'lucide-react'
...
interface LabManualsSelectionData {
  department?: string
  year?: string
  semester?: string
  subject?: SubjectItem
...
export type LabManualsSelectionStepType = 'department' | 'year' | 'semester' | 'subject' | 'upload'

interface LabManualsSelectionStepProps {
  data: Partial<LabManualsSelectionData>
  onNext: (data: Partial<LabManualsSelectionData>) => void
  onBack: () => void
  currentStep: LabManualsSelectionStepType
}

export function LabManualsSelectionStep({ data, onNext, onBack, currentStep }: LabManualsSelectionStepProps) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (currentStep === 'department') {
        ...
        const res = await fetch(`/api/lab-manuals/subjects?dept=${encodeURIComponent(data.department)}&year=${encodeURIComponent(data.year || '2nd year')}&semester=${encodeURIComponent(data.semester)}`, { cache: 'no-store' })
        const json = await res.json()
    ...
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600">
          {currentStep === 'department' && 'Choose department for lab manual upload'}
