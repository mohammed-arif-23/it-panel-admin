...
import { ArrowLeft, FlaskConical, Upload, FileText, Users, TrendingUp, Clock, CheckCircle } from 'lucide-react'
import { LabManualsFlow } from '@/components/lab-manuals/LabManualsFlow'
...
export default function AdminLabManualsPage() {
  ...
  const handleComplete = (data: LabManualData) => {
    console.log('Lab Manual Upload completed:', data)
    setShowFlow(false)
  }
  ...
  const [stats, setStats] = useState([
    { label: 'Total Manuals', value: '0', icon: FileText, color: 'from-blue-500 to-blue-600' },
  ...
  const fetchLabManualStats = async () => {
    setIsLoadingStats(true);
    try {
      // Include credentials for authentication
      const fetchPromises = [
        fetch('/api/admin/lab-manuals/count', { credentials: 'include' }),
        fetch('/api/admin/lab-manuals/departments/count', { credentials: 'include' }),
        fetch('/api/admin/lab-manuals/monthly', { credentials: 'include' }),
        fetch('/api/admin/lab-manuals/success-rate', { credentials: 'include' })
      ];

      const [manualsRes, deptsRes, monthlyRes, successRes] = await Promise.all(fetchPromises);

      // Check if any responses are not ok and provide detailed error info
      const responses = [manualsRes, deptsRes, monthlyRes, successRes];
      ...
      const [manualsData, deptsData, monthlyData, successData] = await Promise.all([
        manualsRes.json(),
        deptsRes.json(),
        monthlyRes.json(),
        successRes.json()
      ...
      setStats([
        { label: 'Total Manuals', value: manualsData.count?.toString() || '0', icon: FileText, color: 'from-blue-500 to-blue-600' },
      ...
      console.error('Failed to fetch Lab Manual stats:', error);
    } finally {
          ...
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center gap-4">
              ...
              <div className="leading-tight">
                <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-slate-900 via-purple-900 to-slate-900 bg-clip-text text-transparent">Lab Manuals</h1>
                <p className="text-sm text-slate-600 font-medium">Advanced Lab Manual Management System</p>
              </div>
            </div>
      ...
      {showFlow ? (
        ...
        <div className="max-w-4xl mx-auto px-4 py-12">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold shimmer-text mb-4">Lab Manual Upload Complete!</h2>
            <p className="text-slate-600 text-lg">Your lab manual has been successfully uploaded and processed.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            <Card className="glass border-white/50 shadow-xl hover:shadow-2xl transition-all duration-500">
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Upload Another Manual</h3>
                <p className="text-slate-600 mb-6">Continue uploading lab manuals for different subjects and departments.</p>
                <button
            ...
            <Card className="glass border-white/50 shadow-xl hover:shadow-2xl transition-all duration-500">
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Manage Manuals</h3>
                <p className="text-slate-600 mb-6">View, edit, or organize your uploaded lab manuals and manage access.</p>
                <Link href="/admin/lab-manuals/manage">
