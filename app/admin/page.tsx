...
export default function AdminRenewLanding() {
  ...
  const fetchRealStats = async () => {
    ...
    try {
      const [studentsRes, assignmentsRes, reviewsRes, systemRes] =
        await Promise.all([
      ...
      const [studentsData, assignmentsData, reviewsDataResult, systemData] =
        await Promise.all([
          studentsRes.json(),
          assignmentsRes.json(),
          reviewsRes.json(),
      ...
      setStats([
        {
          ...
          value: studentsData.count?.toString() || "0",
          icon: Users,
          color: "from-blue-500 to-blue-600",
        },
        {
          label: "Total Assignments",
          value: assignmentsData.count?.toString() || "0",
          icon: FileText,
          color: "from-green-500 to-green-600",
        },
        {
          label: "Pending Reviews",
          value: reviewsDataResult.count?.toString() || "0",
          icon: Clock,
          color: "from-orange-500 to-orange-600",
        },
        {
          label: "System Health",
          value: `${systemData.health || 0}%`,
          icon: Zap,
          color: "from-purple-500 to-purple-600",
        },
      ]);
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="leading-tight">
                  ...
                  <p className="text-sm text-slate-600 font-medium">
                    {userRole === "HOD" ? "Head of Department" : "Staff Member"}{" "}
                    Dashboard
                  </p>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredTiles.map((tile, index) => {
              ...
              const staffRestrictedTiles = [
                "admin/detect-assignments",
                "database",
                "crud",
              ...
              if (
                userRole === "STAFF" &&
                staffRestrictedTiles.includes(tile.id)
              ) {
                return null;
              }
                  ...
                  <Card className="saas-card h-full card-hover border-white/50 bg-white/90 hover:bg-white/95 hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden relative">
                    <CardContent className="p-6 relative z-10">
                      <div className="flex items-start justify-between mb-4">
                        <div
                          className={`h-14 w-14 rounded-2xl bg-gradient-to-br ${tile.color} flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 relative`}
                        >
                          {tile.icon && (
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <span
                            className={`text-xs px-2 py-1 rounded-full bg-gradient-to-r ${tile.color} text-white font-medium opacity-80`}
                          >
                            {tile.category}
                          </span>
                    ...
                    <div
                      className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${tile.color} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left`}
                    ></div>
                  </Card>
                </Link>
              );
