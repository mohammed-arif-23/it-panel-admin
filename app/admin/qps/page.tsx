...
import { ArrowLeft, Library, Upload, FileText, Users, TrendingUp, Clock, CheckCircle } from 'lucide-react'
import { QPFlow } from '@/components/qps/QPFlow'
...
export default function AdminQPsPage() {
  ...
  const handleComplete = (data: QPData) => {
    console.log('QP Upload completed:', data)
    setShowFlow(false)
  }
  ...
  const [stats, setStats] = useState([
    { label: 'Total QPs', value: '0', icon: FileText, color: 'from-blue-500 to-blue-600' },
  ...
  const fetchQPStats = async () => {
    ...
    try {
      ...
      const fetchPromises = [
        fetch('/api/admin/qps/count', { credentials: 'include' }),
        fetch('/api/admin/qps/departments/count', { credentials: 'include' }),
        fetch('/api/admin/qps/monthly', { credentials: 'include' }),
        fetch('/api/admin/qps/success-rate', { credentials: 'include' })
      ];

      const [qpsRes, deptsRes, monthlyRes, successRes] = await Promise.all(fetchPromises);

      // Check if any responses are not ok and provide detailed error info
      const responses = [qpsRes, deptsRes, monthlyRes, successRes];
      ...
      const [qpsData, deptsData, monthlyData, successData] = await Promise.all([
        qpsRes.json(),
        deptsRes.json(),
        monthlyRes.json(),
        successRes.json()
      ...
      setStats([
        { label: 'Total QPs', value: qpsData.count?.toString() || '0', icon: FileText, color: 'from-blue-500 to-blue-600' },
      ...
      console.error('Failed to fetch QP stats:', error);
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-indigo-50/50 page-transition">
          ...
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center gap-4">
              ...
              <div className="leading-tight">
                <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-slate-900 via-purple-900 to-slate-900 bg-clip-text text-transparent">University Question Papers</h1>
                <p className="text-sm text-slate-600 font-medium">Advanced QP Management System</p>
              </div>
            </div>
      ...
      {showFlow ? (
        <div className="relative">
          ...
          <QPFlow onComplete={handleComplete} onCancel={handleCancel} />
        </div>
      ) : (
        <div className="max-w-4xl mx-auto px-4 py-12">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold shimmer-text mb-4">QP Upload Complete!</h2>
            <p className="text-slate-600 text-lg">Your question paper has been successfully uploaded and processed.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
            <Card className="glass border-white/50 shadow-xl hover:shadow-2xl transition-all duration-500">
              <CardContent className="p-6 sm:p-8 text-center">
                <Upload className="h-10 w-10 sm:h-12 sm:w-12 text-violet-600 mx-auto mb-3 sm:mb-4" />
                <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-2 sm:mb-3">Upload Another QP</h3>
                <p className="text-sm sm:text-base text-slate-600 mb-4 sm:mb-6">Continue uploading question papers for different subjects and departments.</p>
                <button
                  onClick={() => setShowFlow(true)}
                  className="w-full bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white py-2.5 sm:py-3 px-4 sm:px-6 rounded-xl text-sm sm:text-base font-semibold hover:shadow-lg transition-all duration-300 hover:scale-105"
                >
                  Start New Upload
                </button>
              </CardContent>
            ...
            <Card className="glass border-white/50 shadow-xl hover:shadow-2xl transition-all duration-500">
              <CardContent className="p-6 sm:p-8 text-center">
                <FileText className="h-10 w-10 sm:h-12 sm:w-12 text-indigo-600 mx-auto mb-3 sm:mb-4" />
                <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-2 sm:mb-3">Manage QPs</h3>
                <p className="text-sm sm:text-base text-slate-600 mb-4 sm:mb-6">View, edit, or organize your uploaded question papers and manage access.</p>
                <Link href="/admin/qps/manage">
                  <button className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-2.5 sm:py-3 px-4 sm:px-6 rounded-xl text-sm sm:text-base font-semibold hover:shadow-lg transition-all duration-300 hover:scale-105">
                    Manage QPs
                  </button>
                </Link>
              </CardContent>
