...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // const count = await db.qps.distinct('department').length;
      ...
      return NextResponse.json({ 
        success: false, 
        count: 0,
        error: 'Failed to fetch departments count' 
      }, { status: 500 });
    }
  } catch (error) {
    console.error('Unexpected error in departments count route:', error);
    return NextResponse.json({ 
      success: false, 
      count: 0,
