...
// GET /api/lab-manuals/list?subject=CODE
export async function GET(req: NextRequest) {
  try {
    ...
    const folderOriginal = `labmanuals/${subject}`
    const folderSanitized = `labmanuals/${sanitized}`
    ...
    return NextResponse.json({ success: false, error: 'Failed to list lab manuals' }, { status: 500 })
  }
}